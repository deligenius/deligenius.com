...
The following example shows how you can put `Mysql` DB(database) instance as a shared context in every middleware. Any item in `state` is supposed to be **read only**.
